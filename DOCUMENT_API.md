...
| `DataIntegrityViolationException` (vi phạm unique/FK ở tầng DB) | **409** | `Duplicate or invalid data submission` |
...
| `/api/jobs/**`, `/api/files/**`, `/uploads/**`, `/api/companies/**`, `/api/categories/**`, `/` | Public |
...
| — | GET | `/api/jobs/category/{categoryId}` | `id,asc` |
| — | GET | `/api/categories` | `id,asc` |
| — | GET | `/api/categories/search?name=` | `id,asc` |
| — | GET | `/api/admin/categories` | `id,asc` |
| — | GET | `/api/admin/categories/active` | `id,asc` |
| — | GET | `/api/recruiter/jobs/category/{categoryId}` | `id,asc` |
| — | GET | `/api/admin/jobs/category/{categoryId}` | `id,asc` |
...
| — | GET | `/api/interviews` | `scheduledAt,desc` |
| — | GET | `/api/recruiter/interviews` | `scheduledAt,desc` |
| — | GET | `/api/recruiter/interviews/application/{applicationId}` | `scheduledAt,desc` |
| — | GET | `/api/admin/interviews` | `scheduledAt,desc` |
| — | GET | `/api/admin/interviews/application/{applicationId}` | `scheduledAt,desc` |
| — | GET | `/api/recruiter/interview-feedbacks` | `createdAt,desc` |
...
| `NotificationType` | `JOB_CREATED`, `APPLICATION_RECEIVED`, `APPLICATION_STATUS_CHANGED`, `APPLICATION_WITHDRAWN`, `INTERVIEW_SCHEDULED`, `INTERVIEW_UPDATED` |
...
| `category` | `CategoryResponse` \| null | Object lồng, **không phải** `categoryId`. `null` với job tạo trước khi có bảng `categories` |
...
> **Bất đối xứng request/response:** request gửi lên `companyId`, `categoryId`, `createdBy` (đều là number), response trả về `company`, `category`, `createdBy` (đều là object).
...
|---|---|---|
| `id` | number | |
| `name` | string | Tên ngành nghề, **unique** (không phân biệt hoa thường) |
| `description` | string \| null | |
| `createdAt` | number | epoch seconds |
| `updatedAt` | number \| null | |
| `isActive` | boolean | `false` = đã ngừng dùng. API public chỉ trả category `isActive = true` |
...
|---|---|---|
| `id` | number | |
| `applicationId` | number | ID đơn ứng tuyển |
| `candidateName` | string | Tên ứng viên (lấy từ `user.fullname`) |
| `jobTitle` | string | Tên vị trí tuyển dụng |
| `title` | string | Tiêu đề lịch phỏng vấn |
| `scheduledAt` | number | Thời điểm bắt đầu (epoch seconds) |
| `endAt` | number \| null | Thời điểm kết thúc |
| `location` | string \| null | Địa điểm phỏng vấn |
| `type` | `InterviewType` \| null | `ONLINE` \| `OFFLINE` |
| `meetingLink` | string \| null | Link meeting (cho phỏng vấn online) |
| `notes` | string \| null | Ghi chú nội bộ. **`null` khi trả về cho ứng viên** — chỉ recruiter/admin mới thấy |
| `result` | `InterviewResult` | `PENDING` \| `INTERVIEWED` \| `PASSED` \| `FAILED` \| `CANCELLED`. Mặc định `PENDING` khi tạo |
| `createdAt` | number | epoch seconds |
| `updatedAt` | number \| null | |

> `notes` được ẩn khỏi response khi ứng viên xem lịch phỏng vấn — recruiter/admin có thể ghi chú nội bộ mà ứng viên không thấy.
...
|---|---|---|
| `id` | number | |
| `interviewId` | number | ID buổi phỏng vấn |
| `interviewTitle` | string | Tiêu đề buổi phỏng vấn |
| `reviewerId` | number | ID user đã viết đánh giá |
| `reviewerName` | string | Tên người đánh giá (`user.fullname`) |
| `candidateName` | string | Tên ứng viên |
| `jobTitle` | string | Tên vị trí tuyển dụng |
| `overallRating` | number | 1–5 |
| `technicalRating` | number \| null | 1–5 |
| `communicationRating` | number \| null | 1–5 |
| `strengths` | string \| null | Điểm mạnh |
| `weaknesses` | string \| null | Điểm yếu |
| `comments` | string \| null | Nhận xét thêm |
| `recommendation` | `FeedbackRecommendation` | `STRONG_HIRE` \| `HIRE` \| `NEUTRAL` \| `NO_HIRE` \| `STRONG_NO_HIRE` |
| `createdAt` | number | epoch seconds |
| `updatedAt` | number \| null | |
...
### 8.4. `GET /api/jobs/category/{categoryId}` — Job theo ngành nghề

**📄 Đã phân trang** (`page`, `size`, `sort` — mặc định `id,asc`).

| Path param | Kiểu |
|---|---|
| `categoryId` | number |

**Response `200`:** `Page<JobResponse>`, chỉ gồm job có `status != CLOSED`.

> `categoryId` không tồn tại → trả **`Page` rỗng**, không phải `404` (endpoint lọc, không phải endpoint tra cứu category).
> Muốn lấy danh sách category để đổ dropdown/bộ lọc, dùng `GET /api/categories` (mục 22).
...
| `categoryId` | number | ✅ | `Category is required`. Với recruiter phải là category **đang hoạt động** (`isActive = true`) — lấy từ `GET /api/categories` (mục 22) |
| `salaryMin` | number | ❌ | ≥ 0 |
...
**Lỗi:** `400` validation / enum sai · `404` `Company not found with id: {id}` · `404` `Category not found` · `409` `Category is no longer available` · `403` sai role.

> ⚠️ **Breaking change:** `categoryId` là bắt buộc. Client cũ không gửi field này sẽ nhận `400` `Request validation failed` với `details.categoryId = "Category is required"`. FE cần gọi `GET /api/categories` để đổ dropdown trước khi submit form tạo/sửa job.

> 🔒 **Recruiter chỉ được chọn category `isActive = true`.** Gửi `categoryId` của category đã bị admin tắt → **`409` `Category is no longer available`**. Vì `GET /api/categories` vốn chỉ trả category đang hoạt động nên dropdown dựng từ đó luôn hợp lệ; lỗi này chỉ xảy ra khi admin tắt category ngay giữa lúc recruiter đang mở form. Ràng buộc này **không áp dụng cho admin** (mục 14).
...
### 13.3b. `GET /api/recruiter/jobs/category/{categoryId}` — Job của mình theo ngành nghề

**📄 Đã phân trang** (mặc định `sort=id,asc`).

Lọc theo `category.id = categoryId` **và** `createdBy = userId` trong token (gồm cả job `CLOSED`).

**Response `200`:** `Page<JobResponse>` — `categoryId` không tồn tại thì trả `Page` rỗng, không phải `404`.
...
**Response `200`:** `JobResponse` · **Lỗi:** `400` validation · `404` `Job not found` / `Company not found with id: {id}` / `Category not found with id: {id}`.
...
| GET | `/api/admin/jobs/category/{categoryId}` | Job theo ngành nghề, không lọc `CLOSED` · **📄 Page** | `page`, `size`, `sort` | `200` `Page<JobResponse>` |
...
> 🔓 **Admin không bị giới hạn category.** Khác recruiter, admin gửi được `categoryId` của cả category đã tắt (`isActive = false`) — hữu ích khi cần sửa lại job cũ đang gắn category đã ngừng dùng mà không phải bật lại category đó.
...
- 🔒 **`OFFER` và `HIRED` yêu cầu đã phỏng vấn đạt:** đơn phải có **ít nhất một lịch phỏng vấn với `result = PASSED`** (xem mục 20). Nếu chưa → **409** `Application must have a passed interview before moving to OFFER` (hoặc `HIRED`).
...
**Lỗi:** `400` `Status is required` · `400` `Malformed request or unsupported enum value` · `404` `Application not found` · `409` sai bước chuyển · `409` `Only the candidate can withdraw an application` · `409` `Application must have a passed interview before moving to OFFER/HIRED`
...
| `INTERVIEW_SCHEDULED` | Candidate | Interview ID | Recruiter/admin tạo lịch phỏng vấn |
| `INTERVIEW_UPDATED` | Candidate | Interview ID | Recruiter/admin cập nhật lịch phỏng vấn |
...
## 20. Interview API — Lịch phỏng vấn

Quản lý lịch phỏng vấn cho ứng viên. Mỗi lịch phỏng vấn gắn với một đơn ứng tuyển (`applicationId`).

API tách theo vai trò: ứng viên dùng `/api/interviews` (chỉ xem), recruiter dùng `/api/recruiter/interviews`, admin dùng `/api/admin/interviews`.

### 20.1. Quy ước quyền sở hữu

| Vai trò | Phạm vi |
|---|---|
| Ứng viên | Chỉ xem lịch phỏng vấn của đơn do chính mình nộp. **Không** thấy `notes` |
| Recruiter | Chỉ CRUD lịch phỏng vấn thuộc đơn ứng tuyển vào job **do chính mình tạo** |
| Admin | Toàn quyền CRUD trên mọi lịch phỏng vấn |

Truy cập lịch không thuộc phạm vi → **`404` `Interview not found`**.

### 20.2. `InterviewRequest`

| Field | Kiểu | Bắt buộc | Validation |
|---|---|:---:|---|
| `applicationId` | number | ✅ | `Application is required` |
| `title` | string | ✅ | Không rỗng, ≤ 200 ký tự |
| `scheduledAt` | number | ✅ | `Scheduled time is required` (epoch seconds) |
| `endAt` | number | ❌ | Phải **sau** `scheduledAt` nếu có |
| `location` | string | ❌ | ≤ 255 ký tự |
| `type` | `InterviewType` | ❌ | `ONLINE` \| `OFFLINE` |
| `meetingLink` | string | ❌ | ≤ 500 ký tự |
| `notes` | string | ❌ | Ghi chú nội bộ (ứng viên không thấy) |

> **Không gửi `result`** — server tự đặt `PENDING` khi tạo. Dùng `PATCH .../interviews/{id}/result` để cập nhật kết quả.
> `endAt` trước `scheduledAt` → **400** `End time must be after scheduled time`.
...
{
  "applicationId": 1,
  "title": "Phỏng vấn vòng 1 — Technical",
  "scheduledAt": 1753190400,
  "endAt": 1753194000,
  "location": "Phòng họp A3",
  "type": "OFFLINE",
  "notes": "Chuẩn bị bài test coding"
}
```

### 20.2b. `InterviewResultRequest`

| Field | Kiểu | Bắt buộc | Validation |
|---|---|:---:|---|
| `result` | `InterviewResult` | ✅ | `Result is required` |

**Ví dụ:** `PATCH /api/recruiter/interviews/5/result`

```json
{ "result": "PASSED" }
```

> Chỉ recruiter (chủ job) và admin được cập nhật kết quả. Ứng viên nhận **thông báo realtime** (`INTERVIEW_UPDATED`) khi kết quả thay đổi.
> 🔗 **Liên kết với luồng trạng thái đơn ứng tuyển:** đơn chỉ chuyển sang `OFFER` / `HIRED` khi có **ít nhất một** lịch phỏng vấn `result = PASSED` (mục 16.1). Vì vậy phải cập nhật `result` thành `PASSED` **trước** khi đổi trạng thái đơn sang `OFFER`.

### 20.3. Candidate — `/api/interviews`

**Quyền:** `ROLE_CANDIDATE`. Chỉ xem, không tạo/sửa/xoá. Response **không** chứa `notes`.

| Method | Path | Mô tả | Response |
|---|---|---|---|
| GET | `/api/interviews` | Lịch phỏng vấn của tôi · 📄 Page | `200` `Page<InterviewResponse>` |
| GET | `/api/interviews/{id}` | Chi tiết lịch phỏng vấn | `200` `InterviewResponse`, `404` |

Sắp xếp mặc định: `sort=scheduledAt,desc`.

### 20.4. Recruiter — `/api/recruiter/interviews`

**Quyền:** `ROLE_RECRUITER` hoặc `ROLE_ADMIN`. Chỉ thấy/thao tác lịch phỏng vấn thuộc job do chính mình tạo.

| Method | Path | Mô tả | Request | Response |
|---|---|---|---|---|
| POST | `/api/recruiter/interviews` | Tạo lịch phỏng vấn | `InterviewRequest` | `201` `InterviewResponse` |
| GET | `/api/recruiter/interviews` | Tất cả lịch phỏng vấn (job của mình) · 📄 Page | `page`, `size`, `sort` | `200` `Page<InterviewResponse>` |
| GET | `/api/recruiter/interviews/{id}` | Chi tiết | — | `200` `InterviewResponse`, `404` |
| GET | `/api/recruiter/interviews/application/{applicationId}` | Lịch theo đơn ứng tuyển (job của mình) · 📄 Page | `page`, `size`, `sort` | `200` `Page<InterviewResponse>` |
| PATCH | `/api/recruiter/interviews/{id}` | Cập nhật lịch phỏng vấn | `InterviewRequest` | `200` `InterviewResponse` |
| PATCH | `/api/recruiter/interviews/{id}/result` | Cập nhật kết quả phỏng vấn | `InterviewResultRequest` | `200` `InterviewResponse` |
| DELETE | `/api/recruiter/interviews/{id}` | Xoá | — | `200` `{"message":"delete success","data":null}`, `404` |
...
| Status | Message | Khi nào |
|---|---|---|
| `400` | Validation errors | Thiếu field bắt buộc, sai format |
| `400` | `End time must be after scheduled time` | `endAt` < `scheduledAt` |
| `400` | `Cannot schedule interview for application with status X` | Application không ở trạng thái `SCREENING` hoặc `INTERVIEW` |
| `404` | `Application not found` | `applicationId` không tồn tại hoặc không thuộc job của mình |

> Khi tạo/cập nhật, ứng viên nhận **thông báo realtime** (`INTERVIEW_SCHEDULED` / `INTERVIEW_UPDATED`).

### 20.5. Admin — `/api/admin/interviews`

**Quyền:** `ROLE_ADMIN`. Không giới hạn phạm vi.

| Method | Path | Mô tả | Request | Response |
|---|---|---|---|---|
| POST | `/api/admin/interviews` | Tạo lịch phỏng vấn | `InterviewRequest` | `201` `InterviewResponse` |
| GET | `/api/admin/interviews` | Tất cả lịch phỏng vấn · 📄 Page | `page`, `size`, `sort` | `200` `Page<InterviewResponse>` |
| GET | `/api/admin/interviews/{id}` | Chi tiết | — | `200` `InterviewResponse`, `404` |
| GET | `/api/admin/interviews/application/{applicationId}` | Lịch theo đơn ứng tuyển · 📄 Page | `page`, `size`, `sort` | `200` `Page<InterviewResponse>` |
| PATCH | `/api/admin/interviews/{id}` | Cập nhật lịch phỏng vấn | `InterviewRequest` | `200` `InterviewResponse` |
| PATCH | `/api/admin/interviews/{id}/result` | Cập nhật kết quả phỏng vấn | `InterviewResultRequest` | `200` `InterviewResponse` |
| DELETE | `/api/admin/interviews/{id}` | Xoá | — | `200` `{"message":"delete success","data":null}`, `404` |

Sắp xếp mặc định: `sort=scheduledAt,desc`.

---

## 21. Interview Feedback API — Đánh giá phỏng vấn

### 21.1. Tổng quan

Sau khi buổi phỏng vấn diễn ra, recruiter (chủ job) ghi lại đánh giá ứng viên: điểm số, điểm mạnh/yếu, nhận xét và đề xuất tuyển dụng.

Đây là **dữ liệu nội bộ** — ứng viên **không có endpoint nào** để xem đánh giá về mình.

**Phân quyền:**

| Vai trò | Phạm vi |
|---|---|
| Recruiter (`/api/recruiter/interview-feedbacks`) | Chỉ đánh giá thuộc buổi phỏng vấn của **job do mình tạo** |
| Admin (`/api/admin/interview-feedbacks`) | Toàn bộ, chỉ đọc + xoá |

Truy cập đánh giá ngoài phạm vi → **`404` `Interview feedback not found`**.

**Quy tắc nghiệp vụ:**

| Quy tắc | Mô tả |
|---|---|
| Mỗi người 1 đánh giá / buổi phỏng vấn | Unique `(interview_id, reviewer_id)` — POST lần 2 → `409` |
| Chỉ tác giả mới sửa/xoá được | Recruiter khác cùng job vẫn xem được nhưng không sửa được |
| Không đánh giá phỏng vấn đã huỷ | `result = CANCELLED` → `409`. Áp dụng cho cả POST lẫn PATCH |
| Không đánh giá trước giờ phỏng vấn | `scheduledAt` còn ở tương lai → `409`. Áp dụng cho cả POST lẫn PATCH |
| Không đổi được `interviewId` | PATCH gửi `interviewId` khác record hiện tại → `409` |

### 21.2. `InterviewFeedbackRequest`

| Field | Kiểu | Bắt buộc | Validation |
|---|---|:---:|---|
| `interviewId` | number | ✅ | `Interview is required` |
| `overallRating` | number | ✅ | 1–5 · `Overall rating must be between 1 and 5` |
| `technicalRating` | number | ❌ | 1–5 · `Technical rating must be between 1 and 5` |
| `communicationRating` | number | ❌ | 1–5 · `Communication rating must be between 1 and 5` |
| `strengths` | string | ❌ | ≤ 2000 ký tự |
| `weaknesses` | string | ❌ | ≤ 2000 ký tự |
| `comments` | string | ❌ | ≤ 2000 ký tự |
| `recommendation` | `FeedbackRecommendation` | ✅ | `Recommendation is required` |

**Ví dụ:** `POST /api/recruiter/interview-feedbacks`
...
```json
{
  ...
  "strengths": "Nền tảng thuật toán tốt, giải quyết bài toán nhanh.",
  "weaknesses": "Diễn đạt còn lúng túng khi trình bày kiến trúc.",
  "comments": "Phù hợp vị trí Backend Middle.",
  "recommendation": "HIRE"
}
```

> `reviewerId` **không gửi lên** — server lấy từ claim `userId` trong JWT.
> `PATCH` dùng cùng schema và cũng yêu cầu đủ các field bắt buộc (ghi đè toàn bộ).

### 21.3. Recruiter — `/api/recruiter/interview-feedbacks`

**Quyền:** `ROLE_RECRUITER`, `ROLE_ADMIN`. Phạm vi giới hạn theo job do mình tạo.

| Method | Path | Mô tả | Request | Response |
|---|---|---|---|---|
| POST | `/api/recruiter/interview-feedbacks` | Tạo đánh giá | `InterviewFeedbackRequest` | `201` `InterviewFeedbackResponse` |
| GET | `/api/recruiter/interview-feedbacks` | Đánh giá thuộc job của mình · 📄 Page | `page`, `size`, `sort` | `200` `Page<InterviewFeedbackResponse>` |
| GET | `/api/recruiter/interview-feedbacks/{id}` | Chi tiết | — | `200` `InterviewFeedbackResponse`, `404` |
| GET | `/api/recruiter/interview-feedbacks/interview/{interviewId}` | Đánh giá theo buổi phỏng vấn · 📄 Page | `page`, `size`, `sort` | `200` `Page<InterviewFeedbackResponse>` |
| PATCH | `/api/recruiter/interview-feedbacks/{id}` | Cập nhật (chỉ tác giả) | `InterviewFeedbackRequest` | `200` `InterviewFeedbackResponse` |
| DELETE | `/api/recruiter/interview-feedbacks/{id}` | Xoá (chỉ tác giả) | — | `200` `{"message":"delete success","data":null}`, `404` |

**Lỗi thường gặp:**

| Mã | Message | Nguyên nhân |
|---|---|---|
| `404` | `Interview not found` | Buổi phỏng vấn không tồn tại hoặc không thuộc job của mình |
| `404` | `Interview feedback not found` | Đánh giá không tồn tại hoặc ngoài phạm vi |
| `409` | `Feedback for this interview has already been submitted` | Đã đánh giá buổi phỏng vấn này rồi |
| `409` | `Cannot give feedback for a cancelled interview` | `result = CANCELLED` |
| `409` | `Cannot give feedback before the interview takes place` | `scheduledAt` còn ở tương lai |
| `409` | `Only the reviewer can modify this feedback` | Sửa/xoá đánh giá của người khác |
| `409` | `Interview cannot be changed on an existing feedback` | PATCH gửi `interviewId` khác record hiện tại |
| `409` | `Duplicate or invalid data submission` | Hai request tạo đánh giá chạy đua cùng lúc (vi phạm unique constraint) |

Sắp xếp mặc định: `sort=createdAt,desc`.

> Các lỗi nghiệp vụ ở module này trả **`409 Conflict`** (`BusinessRuleException`), thống nhất với Application/Auth/User API — khác với Interview API (mục 20) vốn dùng `IllegalArgumentException` → `400`.

### 21.4. Admin — `/api/admin/interview-feedbacks`

**Quyền:** `ROLE_ADMIN`. Không giới hạn phạm vi, chỉ đọc và xoá (admin không tự viết đánh giá).

| Method | Path | Mô tả | Request | Response |
|---|---|---|---|---|
| GET | `/api/admin/interview-feedbacks` | Tất cả đánh giá · 📄 Page | `page`, `size`, `sort` | `200` `Page<InterviewFeedbackResponse>` |
| GET | `/api/admin/interview-feedbacks/{id}` | Chi tiết | — | `200` `InterviewFeedbackResponse`, `404` |
| GET | `/api/admin/interview-feedbacks/interview/{interviewId}` | Đánh giá theo buổi phỏng vấn · 📄 Page | `page`, `size`, `sort` | `200` `Page<InterviewFeedbackResponse>` |
| DELETE | `/api/admin/interview-feedbacks/{id}` | Xoá | — | `200` `{"message":"delete success","data":null}`, `404` |

Sắp xếp mặc định: `sort=createdAt,desc`.

---

## 22. Category API — Ngành nghề của job

### 22.1. Tổng quan

`categories` là bảng riêng trong database. Mỗi job thuộc **tối đa 1 category** (`jobs.category_id` → `categories.id`, quan hệ ManyToOne).

Category do **admin tạo trước**; recruiter/admin khi tạo job chỉ **chọn từ danh sách có sẵn** bằng `categoryId` — đúng cơ chế đang dùng cho `companyId`. Gửi `categoryId` không tồn tại → **`404` `Category not found with id: X`**.

| Vai trò | Phạm vi |
|---|---|
| Ứng viên (`/api/categories`) | Chỉ đọc, **chỉ thấy category `isActive = true`** |
| Recruiter | Dùng chung `/api/categories` (không có endpoint riêng, giống cách recruiter dùng `/api/companies`) → cũng **chỉ thấy và chỉ chọn được category `isActive = true`** khi tạo/sửa job |
| Admin (`/api/admin/categories`) | CRUD đầy đủ, thấy cả category đã ngừng dùng, và **gắn được category đã tắt vào job** |

> Recruiter vẫn **đọc** được job cũ của mình đang gắn category đã tắt — `JobResponse.category` trả đầy đủ, và `GET /api/recruiter/jobs/category/{categoryId}` vẫn lọc được theo category đã tắt. Giới hạn `isActive` chỉ áp cho việc **chọn category khi tạo/sửa job**, không làm mất dữ liệu đã có.

**Quy tắc nghiệp vụ:**

| Quy tắc | Mô tả |
|---|---|
| Tên category là unique | Không phân biệt hoa thường. Trùng → `409` `Category name already exists` |
| Không xoá cứng | Chỉ bật/tắt qua `isActive` — tránh làm mồ côi job đang trỏ tới |
| Recruiter chỉ chọn được category đang mở | `isActive = false` → `409` `Category is no longer available`. Admin không bị chặn |
| Job cũ không có category | `jobs.category_id` nullable, job tạo trước khi có bảng này trả `category: null` |

### 22.2. `CategoryRequest`

| Field | Kiểu | Bắt buộc | Validation |
|---|---|:---:|---|
| `name` | string | ✅ | `Category name is required`, ≤ 100 ký tự |
| `description` | string | ❌ | ≤ 255 ký tự |

```json
{
  "name": "Công nghệ thông tin",
  "description": "Lập trình, hạ tầng, dữ liệu"
}
```

> Không gửi `isActive` — server tự đặt `true` khi tạo. Dùng `PATCH .../{id}/active` hoặc `.../{id}/deactivate` để đổi.

### 22.3. Public / Ứng viên — `/api/categories`

**Quyền:** Public (nằm trong whitelist của `SecurityConfig`), chỉ trả category `isActive = true`.

| Method | Path | Mô tả | Request | Response |
|---|---|---|---|---|
| GET | `/api/categories` | Danh sách ngành nghề đang dùng · 📄 Page | `page`, `size`, `sort` | `200` `Page<CategoryResponse>` |
| GET | `/api/categories/search?name=` | Tìm theo tên (khớp chuỗi con, bỏ qua hoa thường) · 📄 Page | `name`, `page`, `size`, `sort` | `200` `Page<CategoryResponse>` |
| GET | `/api/categories/{id}` | Chi tiết | — | `200` `CategoryResponse`, `404` |
...
**Lỗi:** `404` `Category not found` — id không tồn tại **hoặc** category có `isActive = false` (trả 404 để không lộ sự tồn tại của category đã ngừng dùng, giống cách xử lý của Company và Job).

Sắp xếp mặc định: `sort=id,asc`.

### 22.4. Admin — `/api/admin/categories`

**Quyền:** `ROLE_ADMIN`.

| Method | Path | Mô tả | Request | Response |
|---|---|---|---|---|
| POST | `/api/admin/categories` | Tạo ngành nghề | `CategoryRequest` | `201` `CategoryResponse` |
| GET | `/api/admin/categories` | Tất cả (kể cả đã ngừng dùng) · 📄 Page | `page`, `size`, `sort` | `200` `Page<CategoryResponse>` |
| GET | `/api/admin/categories/active` | Chỉ category đang dùng · 📄 Page | `page`, `size`, `sort` | `200` `Page<CategoryResponse>` |
| GET | `/api/admin/categories/{id}` | Chi tiết (kể cả đã ngừng dùng) | — | `200` `CategoryResponse`, `404` |
| PATCH | `/api/admin/categories/{id}` | Cập nhật tên/mô tả | `CategoryRequest` | `200` `CategoryResponse` |
| PATCH | `/api/admin/categories/{id}/deactivate` | Ngừng dùng | — | `200` `CategoryResponse` |
| PATCH | `/api/admin/categories/{id}/active` | Dùng lại | — | `200` `CategoryResponse` |

**Lỗi thường gặp:**

| Mã | Message | Nguyên nhân |
|---|---|---|
| `404` | `Category not found` | id không tồn tại |
| `409` | `Category name already exists` | Trùng tên với category khác (không phân biệt hoa thường) |

Sắp xếp mặc định: `sort=id,asc`.

> Ngừng dùng một category **không** ảnh hưởng job đang trỏ tới nó — job vẫn trả về `category` đầy đủ trong `JobResponse`. Category đó chỉ biến mất khỏi `/api/categories` và recruiter không chọn được nữa (`409` `Category is no longer available`); admin vẫn gắn được.
